...
| `prompt_recording_disclaimer` | `POST /api/rooms/{room_id}/recording/prompt-disclaimer` | None | `{message}` | SDK nonzero HTTP 500 with `{detail:{message,error_code,error_name}}` | Exact after structured-error fix |
| `ai_companion_on` | `POST /api/rooms/{room_id}/ai-companion/turn-on` | Query `features:int` | `{room_id, features, result:0, success:true}` | SDK nonzero HTTP 502 with `{detail:{message,error_code,error_name}}` | Exact after structured-error fix |
| `ai_companion_off` | `POST /api/rooms/{room_id}/ai-companion/turn-off` | Query `features:int`, `delete_assets:bool` | `{room_id, features, delete_assets, result:0, success:true}` | SDK nonzero HTTP 502 with `{detail:{message,error_code,error_name}}` | Exact after structured-error fix |
...
| `mute_audio(true)` | `POST /api/rooms/{room_id}/audio/mute` | None | `{room_id, muted:true, result, success:true}` | SDK result 10 (`ALREADY_IN_THIS_STATE`) is idempotent success; other nonzero results use `success:false`; legacy state queries 422 | Exact desired-state setter |
| `mute_audio(false)` | `POST /api/rooms/{room_id}/audio/unmute` | None | `{room_id, muted:false, result, success:true}` | SDK result 10 (`ALREADY_IN_THIS_STATE`) is idempotent success; other nonzero results use `success:false`; legacy state queries 422 | Exact desired-state setter |
...
| `mute_video(true)` | `POST /api/rooms/{room_id}/video/mute` | None | `{room_id, muted:true, result, success:true}` | SDK result 10 (`ALREADY_IN_THIS_STATE`) is idempotent success; other nonzero results use `success:false`; legacy state queries 422 | Exact desired-state setter |
| `mute_video(false)` | `POST /api/rooms/{room_id}/video/unmute` | None | `{room_id, muted:false, result, success:true}` | SDK result 10 (`ALREADY_IN_THIS_STATE`) is idempotent success; other nonzero results use `success:false`; legacy state queries 422 | Exact desired-state setter |
